...
#define HTTPNewCarDetailParements(carID) [NSString stringWithFormat:@"http://s.new.shenchenet.com/mobile/?act=index&op=android_api&android_newcars_para=1&did=%@",(carID)]
...
/*二手车*/
#define HTTPOldCarRecommend @"http://used.shenchenet.com/?act=index&android_usedcar=1"//二手车推荐:
...
#define HTTPImportCarRecommend @"http://import.shenchenet.com/?act=index&android_importcar=1"//进口车推荐


#define HTTPDetailImgs(id,pre) [NSString stringWithFormat:@"http://s.new.shenchenet.com/mobile/?act=index&op=getCarImages&id=%@&pre=%@&android_pic=1",(id),(pre)]//@"http://s.new.shenchenet.com/mobile/?act=index&op=getCarImages&id=105984&pre=new&android_pic=1"

#define HTTPInstallMentOrderRequest(member,type) [NSString stringWithFormat:@"http://s.new.shenchenet.com/mobile/?act=index&op=getMortDataAndroid&member=%@&type=%@&android_pass=1",(member),(type)]//http://s.new.shenchenet.com/mobile/?act=index&op=getMortDataAndroid&member=?&type=?&android_pass=1 按揭列表

#define HTTPInstallMentPost @"http://s.new.shenchenet.com/mobile/"//
